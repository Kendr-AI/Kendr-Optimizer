...
export async function handleClaudeCodeHook(pathname, payload, options = {}) {
    try {
        ...
        if (pathname === "/hooks/claude-code/stop") {
            await shadowText(payload.last_assistant_message, "assistant", "output_observation", payload, client);
            return {};
        }
    }
    catch {
...
async function handlePostToolUse(payload, client, shadow) {
...
async function shadowText(text, role, phase, payload, client) {
    if (typeof text !== "string" || text.length === 0)
        return;
    const requestId = "claude-code-shadow-" + randomUUID();
    await client.analyze(buildRequest({
        requestId,
        sessionId: nonEmptyString(payload.session_id),
        phase,
        shadow: true,
...
function collectTextBindings(value, toolUseId) {
    const bindings = [];
    if (typeof value === "string") {
        if (value.length > 0)
            bindings.push({ path: [], original: value, callId: toolUseId + ":0" });
        return bindings;
    }
    visitText(value, [], undefined, bindings, toolUseId);
    return bindings;
}
function visitText(value, path, key, bindings, toolUseId) {
    if (typeof value === "string") {
...
function isTextBearingKey(key) {
    return /^(stdout|stderr|output|content|text|result|message|error|body|log|logs|diff|patch|transcript|summary|response)$/i.test(key);
}
function decodeToolResultOutcome(raw, requestId, messageId, bindings) {
    if (!isCoreOutcome(raw))
        return undefined;
    if (raw.receipt.schema_version !== "kendr.receipt/v1")
        return undefined;
    if (raw.receipt.request_id !== requestId ||
        raw.receipt.status !== "applied" ||
        raw.receipt.token_delta <= 0 ||
        raw.receipt.original.tokens <= raw.receipt.optimized.tokens) {
        return undefined;
    }
    if (raw.content.messages.length !== 1)
        return undefined;
    const message = raw.content.messages[0];
    if (!message || message.id !== messageId || message.role !== "tool")
        return undefined;
    ...
    for (let index = 0; index < bindings.length; index += 1) {
        ...
        if (!part || !binding || part.type !== "tool_result")
            return undefined;
...
function applyTextBindings(original, bindings, replacements) {
    if (bindings.length !== replacements.length)
        return undefined;
    if (bindings.length === 1 && bindings[0]?.path.length === 0)
...
function isCoreOutcome(value) {
    ...
    return (typeof value.receipt.schema_version === "string" &&
...
export function createClaudeCodeBridgeServer(options = {}) {
...
async function serveHookRequest(request, response, options, maxBodyBytes) {
    ...
    try {
        ...
        const output = isRecord(body)
            ? await handleClaudeCodeHook(pathname, body, options)
            : {};
...
function isKnownPath(pathname) {
    return (pathname === "/hooks/claude-code/post-tool-use" ||
