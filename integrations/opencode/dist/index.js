...
export class KendrOpenCodeAdapter {
    ...
    async optimizeCurrentMessage(parts, sessionId, messageId, model) {
        ...
        const request = this.#buildRequest("request", requestId, encoded.messages, sessionId, model ? model.providerID + "/" + model.modelID : undefined, { seam: "chat.message", message_id: messageId ?? null });
    ...
    async optimizeToolOutput(output, input) {
        ...
        ], input.sessionID, undefined, { seam: "tool.execute.after", tool: input.tool });
        const raw = await this.#dispatch(request);
        if (this.#shadow)
            return undefined;
    ...
    async optimizeHistory(messages) {
        ...
        const request = this.#buildRequest("history_ingest", requestId, encoded.messages, undefined, undefined, { seam: "experimental.chat.messages.transform", experimental: true });
        const raw = await this.#dispatch(request);
        if (this.#shadow)
            return undefined;
        const replacements = decodePartOutcome(raw, requestId, encoded);
    ...
    async optimizeSystem(system, sessionId, model) {
        ...
        const request = this.#buildRequest("request", requestId, encoded.messages, sessionId, modelName(model), { seam: "experimental.chat.system.transform", experimental: true });
        const raw = await this.#dispatch(request);
        if (this.#shadow)
            return undefined;
        const replacements = decodePartOutcome(raw, requestId, encoded);
    ...
    return async () => {
        ...
        const hooks = {
            "chat.message": async (input, output) => {
                try {
                    const parts = await adapter.optimizeCurrentMessage(output.parts, input.sessionID, input.messageID, input.model);
                    if (parts)
                        output.parts = parts;
                }
                catch {
...
export const KendrOptimizerPlugin = async (input, pluginOptions) => {
