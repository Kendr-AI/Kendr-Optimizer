...
export type ChannelOptimizationReason =
  | "optimized"
  | "sender_not_authorized"
  | "empty_content"
  | "shadow_only"
  | "optimizer_unavailable"
  | "not_applied_or_invalid"
  | "unchanged";
...
export class KendrClaudeChannelOptimizer {
  ...
  async prepareNotification<T extends ChannelNotification>(
    notification: T,
    context: ChannelMessageContext,
  ): Promise<ChannelOptimizationResult<T>> {
    ...
    const optimized = { ...notification, content: replacement } as T;
...
export function createClaudeChannelOptimizer(
  options: ClaudeChannelOptimizerOptions = {},
): KendrClaudeChannelOptimizer {
...
function decodeTextOutcome(raw: unknown, requestId: string, messageId: string): string | undefined {
  if (!isRecord(raw) || !isRecord(raw.receipt) || !isRecord(raw.content)) return undefined;
  if (!isRecord(raw.receipt.original) || !isRecord(raw.receipt.optimized)) return undefined;
  if (
    raw.receipt.schema_version !== "kendr.receipt/v1" ||
    ...
    (raw.receipt.token_delta as number) <= 0 ||
    !Number.isSafeInteger(raw.receipt.original.tokens) ||
    !Number.isSafeInteger(raw.receipt.optimized.tokens) ||
    (raw.receipt.original.tokens as number) <= (raw.receipt.optimized.tokens as number)
  ) {
    return undefined;
  }
  if (!Array.isArray(raw.content.messages) || raw.content.messages.length !== 1) return undefined;
  const message = raw.content.messages[0];
  if (!isRecord(message) || message.id !== messageId || message.role !== "user") return undefined;
