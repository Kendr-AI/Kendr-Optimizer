...
class ServiceCircuit {
    ...
    failedAttempt(reason) {
        if (!this.failed || Date.now() >= this.retryAfter) {
            this.logger.warn("KendrOptimizer service unavailable; using the original OpenClaw context. " + reason);
        }
        this.failed = true;
...
class KendrClient {
    ...
    optimizeUrl;
    circuit;
    constructor(endpoint, timeoutMs, backoffMs, logger) {
...
class CommitRegistry {
    ...
    async persist() {
        ...
        try {
            await writeFile(temporary, JSON.stringify({ schema: 1, keys: [...this.keys].sort() }) + "\n", { encoding: "utf8", mode: 0o600 });
...
export default function register(api) {
    api.registerContextEngine(PLUGIN_ID, (factoryContext) => {
        ...
        const stateRoot = factoryContext.agentDir ?? factoryContext.workspaceDir ?? process.cwd();
        const commitRegistry = new CommitRegistry(join(stateRoot, ".kendr-optimizer", "openclaw-commits-v1.json"));
        const client = new KendrClient(config.endpoint, config.timeoutMs, config.failureBackoffMs, api.logger);
        return {
            info: {
                id: PLUGIN_ID,
                name: "KendrOptimizer",
            ...
            async assemble(params) {
                ...
                try {
                    ...
                    api.logger.warn("KendrOptimizer skipped an unsupported OpenClaw message shape: " +
                        formatError(error));
                    return {
                ...
                const model = params.runtimeSettings?.model?.resolved ?? params.model ?? null;
                const contextLimit = params.tokenBudget ??
                    params.runtimeSettings?.limits?.promptTokenBudget ??
                    null;
                const request = makeRequest(config, params.sessionId, model, contextLimit, encoded.messages);
                ...
                try {
                    ...
                    const receiptTokens = outcome.receipt.status === "shadow"
                        ? outcome.receipt.original.tokens
                        : outcome.receipt.optimized.tokens;
                    api.logger.debug("KendrOptimizer " +
                    ...
                    api.logger.warn("KendrOptimizer returned an incompatible message shape; using original context. " +
                        formatError(error));
                    return {
                        messages: params.messages,
                        estimatedTokens: originalEstimate,
            ...
            async compact() {
                return {
                    ok: false,
                    compacted: false,
                    reason: "KendrOptimizer is a transform-only engine and does not own OpenClaw transcript compaction.",
                };
            },
...
function makeRequest(config, sessionId, model, contextLimit, messages) {
    return {
    ...
    input.forEach((original, messageIndex) => {
        ...
        if (typeof content === "string") {
            if (role === "tool" && toolCallId(original) !== null) {
                ...
                const part = { type: "text", text: content };
                parts.push(part);
                partBindings.push({
                    kind: "content_string",
                    original: { ...part },
            ...
            throw new Error("message " + messageIndex + " content is neither a string nor an array");
        }
        messages.push({
            id,
            role,
...
function decodeMessages(optimized, bindings) {
    ...
    return optimized.map((message, messageIndex) => {
        ...
        if (!binding.contentWasArray) {
            ...
            const text = partBinding.original.type === "text"
...
function returnedTextPart(part) {
    if (part.type !== "text") {
        throw new Error("text part changed type");
    }
    return part.text;
}
function returnedToolResult(part, original) {
    if (part.type !== "tool_result") {
        throw new Error("tool-result part changed type");
    }
    if (part.call_id !== original.call_id ||
...
function parseOutcome(value) {
    ...
    if (!isRecord(value.receipt.original) ||
...
function parseKendrMessage(value) {
    if (!isRecord(value) ||
...
function parseConfig(input) {
    ...
    return {
        ...
        failureBackoffMs: readInteger(input, "failureBackoffMs", 5000, 0, 60000),
        tokenizerProfile: readEnum(input, "tokenizerProfile", ["approximate", "cl100k_base", "o200k_base"], "approximate"),
        riskCeiling: readEnum(input, "riskCeiling", ["pass_through", "representation_safe"], "representation_safe"),
        minGainTokens: readInteger(input, "minGainTokens", 8, 0, 1_000_000),
        minGainPercent: readNumber(input, "minGainPercent", 1, 0, 100),
        preserveRecentMessages: readInteger(input, "preserveRecentMessages", 6, 0, 1000),
        maxToolResultChars: readInteger(input, "maxToolResultChars", 24_000, 256, 10_000_000),
        shadow: readBoolean(input, "shadow", false),
    };
}
function normalizeLoopbackOrigin(input) {
    let url;
    try {
        url = new URL(input);
    }
    ...
    if (hostname !== "127.0.0.1" &&
        hostname !== "[::1]") {
        throw new Error("endpoint must use the numeric loopback address 127.0.0.1 or [::1]");
    }
    if (url.username !== "" ||
        url.password !== "" ||
...
function positiveTokenEstimate(receiptTokens, messages) {
...
function readNumber(input, key, fallback, minimum, maximum) {
    const value = input[key];
    if (value === undefined) {
        return fallback;
    }
...
function readEnum(input, key, allowed, fallback) {
    const value = input[key];
    if (value === undefined) {
        return fallback;
    }
