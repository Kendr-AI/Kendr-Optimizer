...
def unqualified_reason(row: dict[str, Any]) -> str:
    ...
    if not row["full_surface_coverage"]:
        reasons.append(
            "full-surface coverage failed "
            "({completed}/{expected} cases completed, {eligible}/{expected} declared "
            "eligible, {failed} failed)".format(
    ...
    if row["preservation_proxy_passed"] < row["completed_cases"]:
        reasons.append(
            "fixture-preservation gate failed "
            "({passed}/{completed} completed cases passed)".format(
                passed=row["preservation_proxy_passed"],
                completed=row["completed_cases"],
            )
        )
    elif row["completed_cases"] == 0:
        reasons.append("fixture-preservation gate unevaluable (0 cases completed)")
    elif row["full_surface_coverage"] and not row["all_preservation_proxies_pass"]:
        reasons.append(
            "source preservation-complete flag failed despite fixture counts "
            "({passed}/{completed} completed cases passed)".format(
                passed=row["preservation_proxy_passed"],
                completed=row["completed_cases"],
            )
        )
    return "; ".join(reasons) or "excluded by primary ranking policy"
...
def build_ranking(
    ...
    payload = {
        ...
        "methodology": {
            ...
            "raw_reduction_formula": (
                "100 * (sum(completed-case input tokens) - "
                "sum(completed-case output tokens)) / "
                "sum(completed-case input tokens)"
            ),
            "raw_aggregation_scope": (
                "Only successfully completed cases enter the raw aggregate. Failed and "
                "unsupported cases contribute no tokens to its numerator or denominator; "
                "coverage counts are reported beside it."
            ),
            "source_release_summary_qualification": (
                "The source release summary qualifies a row over that optimizer's declared "
                "eligible cases when all declared eligible cases complete and every completed "
                "case passes the fixture-preservation gate."
            ),
            "public_primary_ranking_gate": (
                "The public ranking is stricter: the configuration must declare eligible for "
                "and complete every frozen corpus case on the surface, have zero failures, "
                "and pass the fixture-preservation gate on every completed case."
            ),
            "qualified_reduction_rule": (
                "Qualified token reduction equals raw token reduction exactly when the public "
                "primary ranking gate passes; otherwise it is null and the row is excluded "
                "from the primary rank."
            ),
            "fixture_preservation_gate": {
                "hard_requirements": [
                    "every fixture-declared required literal is retained exactly",
                    "the primary payload is semantically equal JSON where the fixture requires JSON equivalence",
                    "the benchmark query marker is retained exactly",
                ],
                "diagnostics_only_unless_declared_required": [
                    "URL recall",
                    "path recall",
                    "number recall",
                ],
                "quality_notice": (
                    "This composite fixture gate detects specified corruption; it is not a "
                    "downstream target-model or task-quality evaluation."
                ),
            },
            "primary_eligibility": [
                "declared eligible for and completed every frozen corpus case assigned to the surface",
                "zero failed cases",
                "every completed case passed the composite fixture-preservation gate",
                "KendrOptimizer is represented only by its default shipped configuration",
            ],
            "ordering": [
                "higher qualified token reduction",
                "fewer completed cases with zero token delta",
            ...
            "excluded_from_score": [
                "diagnostic raw reductions from rows excluded by the public gate",
...
def render_markdown(payload: dict[str, Any]) -> str:
    ...
    prompt_cases = payload["tracks"]["prompt_context"]["expected_cases"]
    tool_cases = payload["tracks"]["tool_output"]["expected_cases"]
    lines = [
        ...
        "to this authored corpus and its fixture-preservation checks; they do not establish target-"
        ...
        "```",
        "",
        "Failed and unsupported cases contribute no tokens to that numerator or denominator. "
        "The report therefore always shows completion and eligibility counts beside a raw "
        "percentage. A partial-run raw percentage is diagnostic, not evidence of full-surface "
        "performance.",
        "",
        "**Qualified token reduction** is not a discounted or independently recalculated "
        "score. It equals the raw token reduction exactly when the public primary-ranking "
        "gate passes. Otherwise its value is `null`/`N/A`, and the configuration is excluded "
        "from the primary rank even though its diagnostic raw reduction remains visible.",
        "",
        "An exclusion says that this exact configuration did not satisfy this corpus gate on "
        "this surface. It does not, by itself, claim that the optimizer can never preserve "
        "quality on another workload.",
        "",
        "## How this release is tested",
        "",
        f"1. Freeze the corpus, configuration, source revisions, and tokenizer. This release "
        f"contains {prompt_cases} prompt/context cases and {tool_cases} tool-output cases.",
        "2. Execute one optimizer configuration on every case assigned to its surface and "
        "retain the raw input, output, status, timing, and score artifacts.",
        "3. Independently recount input and output tokens for every successfully completed "
        "case with the tokenizer recorded under Scope and provenance.",
        "4. Compute the diagnostic raw aggregate from those completed cases only. Preserve "
        "failed and unsupported cases in the coverage ledger rather than treating them as "
        "zero-token successes.",
        "5. Apply one composite fixture-preservation gate to each completed case. It requires "
        "every fixture-declared literal, semantic JSON equality where declared, and the exact "
        "benchmark query marker. URL, path, and number recall are diagnostics unless the "
        "fixture also declares those values as required literals.",
        "6. Apply the public full-surface gate: every frozen case must be declared eligible "
        "and completed, failures must be zero, and every completed case's fixture gate must "
        "pass.",
        "7. Copy the raw percentage unchanged into Qualified token reduction only after that "
        "gate passes; otherwise emit `null`/`N/A` and show the row only in the diagnostic "
        "excluded table.",
        "",
        "The release summary has a narrower, source-level qualification field: it checks "
        "completion and fixture gates over the optimizer's *declared eligible cases*. The "
        "public ranking deliberately strengthens that rule to every frozen corpus case on "
        "the surface, preventing an optimizer from improving rank by declaring hard cases "
        "unsupported. The fixture gate catches specified corruption; it is not a downstream "
        "target-model or task-native quality evaluation.",
        "",
        "## Primary ranking rules",
        "",
        "A configuration enters a primary table only when it completed every corpus case "
        "on that surface with zero failures and every composite fixture-preservation gate passed. "
        "Rows are ordered by higher qualified token reduction and then fewer completed cases "
        "with zero token delta. Configurations tied on both share a standard competition rank (for example, "
    ...
    for surface in SURFACES:
        ...
        lines.extend(
            [
                ...
                "",
                f"Full-surface coverage is {track['expected_cases']} cases.",
                "",
                ...
                "failed fixture-preservation gates.",
                "",
