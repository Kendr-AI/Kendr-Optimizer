...
export class KendrPiAdapter {
  ...
  async optimizeSystemPrompt(
    ...
    if (event.systemPrompt.length === 0) return undefined;
  ...
  async optimizeContext(
    ...
    const encoded = encodeContext(event.messages as unknown[]);
  ...
  async optimizeToolResult(
    event: ToolResultEvent,
    context: ExtensionContext,
  ): Promise<ToolResultEvent["content"] | undefined> {
    const textBlocks: Array<{ index: number; text: string; callId: string }> = [];
    ...
    const parts: CorePart[] = textBlocks.map((block) => ({
      type: "tool_result",
    ...
    for (let index = 0; index < textBlocks.length; index += 1) {
      ...
      if (block.text !== replacement) changed = true;
      block.text = replacement;
    }
    return changed ? copy : undefined;
  }

  async observeAssistantOutput(
    ...
    if (encoded.bindings.length === 0) return;
  ...
  #buildRequest(
    phase: "request" | "tool_result" | "history_ingest" | "output_observation",
    requestId: string,
    messages: CoreMessage[],
    context: ExtensionContext,
    forceShadow = false,
  ): JsonRecord {
    const target: JsonRecord = { tokenizer_profile: "approximate" };
    const model = context.model;
    if (model?.id) target.model = model.id;
...
export function createPiOptimizerExtension(
  options: PiOptimizerOptions = {},
...
function optionsFromEnvironment(): PiOptimizerOptions {
  const options: PiOptimizerOptions = {};
  ...
  if (process.env.KENDR_OPTIMIZER_TIMEOUT_MS) {
    ...
    if (Number.isSafeInteger(timeoutMs) && timeoutMs > 0) options.timeoutMs = timeoutMs;
  }
...
function encodeContext(sourceMessages: unknown[]): EncodedContext {
  ...
  for (let sourceIndex = 0; sourceIndex < sourceMessages.length; sourceIndex += 1) {
    ...
    const pending: Array<Omit<ContextBinding, "coreMessageIndex">> = [];
...
function decodeContextOutcome(
  ...
  for (let index = 0; index < encoded.bindings.length; index += 1) {
    ...
    if (!isRecord(part) || part.type !== binding.coreType) return undefined;
...
function applyContextBindings<T extends readonly unknown[]>(
  sourceMessages: T,
  bindings: ContextBinding[],
  replacements: string[],
): T | undefined {
  if (bindings.length !== replacements.length) return undefined;
  const copy = structuredClone(sourceMessages) as T;
  let changed = false;
  for (let index = 0; index < bindings.length; index += 1) {
    ...
    if (!setAtPath(message, binding.sourcePath, replacement)) return undefined;
    if (replacement !== binding.original) changed = true;
  }
  return changed ? copy : undefined;
}

function decodeSingleText(
  raw: unknown,
  requestId: string,
  messageId: string,
  role: string,
  ...
  if (!isRecord(message) || message.id !== messageId || message.role !== role) return undefined;
...
function decodeToolParts(
  raw: unknown,
  requestId: string,
  messageId: string,
  bindings: Array<{ callId: string }>,
): string[] | undefined {
  const outcome = parseLocallyAppliedOutcome(raw, requestId);
  if (!outcome || outcome.content.messages.length !== 1) return undefined;
  const message = outcome.content.messages[0];
  ...
  for (let index = 0; index < bindings.length; index += 1) {
    ...
    if (!binding || !isRecord(part) || part.type !== "tool_result") return undefined;
    if (part.call_id !== binding.callId || typeof part.content !== "string") return undefined;
    replacements.push(part.content);
  }
  return replacements;
...
function mapPiRole(value: unknown): CoreMessage["role"] | undefined {
...
function safeSessionId(context: ExtensionContext): string | undefined {
  try {
